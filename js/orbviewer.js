...
const minX = -1.0993153024260256e10 / scale;
const maxX = 1.1259105381765476e10 / scale;
const minY = -8.336972753734525e9 / scale;
const maxY = 1.1216725295000006e10 / scale;
const minZ = -5.482463379824468e9 / scale;
const maxZ = 4.381383003697839e9 / scale;
...
const colourmaps = [Rainbow, Trans, Bi, White];
...
function init() {
  ...
  if (!renderer.vr._origGetCamera)
    renderer.vr._origGetCamera = renderer.vr.getCamera;
  ...
  for (let i = 0; i < n; i++) {
    if (lines[i] !== "") {
      ...
      if (Array.isArray(color)) {
        // if our color is an array we probably need to pick one of the entries.
        let c = mapToColour(color, x, y, z);
        colors.push(c[0], c[1], c[2]);
      } else {
        // otherwise it should be a three.Color object.
...
function raycastCheck() {
  ...
  if (intersection !== null) {
    ...
    linkTag.href =
      "https://www.minorplanetcenter.net/db_search/show_object?utf8=✓&object_id=" +
      objectID;
    if (server) {
      const loader = new THREE.FileLoader();
      loader.load(
        "/obj/" + objectID.replace(/ /g, "+"),
        function (data) {
          if (orbitLine !== null) {
          ...
          for (let i = 0; i < response.Orbit.length; i++) {
            positions.push(
              response.Orbit[i].X / scale,
              response.Orbit[i].Z / scale,
              response.Orbit[i].Y / scale,
            );
            colors.push(0, 0, 255);
          }
          ...
          orbitLine = new THREE.Line(
            geometry,
            new THREE.LineBasicMaterial({ color: 0x77ff77, linewidth: 10 }),
          );
          scene.add(orbitLine);
...
function needsResize(canvas) {
  if (
    canvas.lastWidth !== canvas.clientWidth ||
    canvas.lastHeight !== canvas.clientHeight
  ) {
